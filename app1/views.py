        ...
        if 'PassButton' in request.POST:
            ...
            if len(Customer) <= 10:
                ...
                if len(Project_Unit) == 10:
                    ...
                    ResultTime = request.POST.get('ResultTime')
                    if StartTime < EndTime and EndTime < ResultTime:
                        if len(Phase) <= 10:
                            Testitem = request.POST.get('item')
                            dic_i = {'Item_I': Testitem}
                            # print(dic_i)
                            # dic_i = {'Item_I': "A39MEC00715"}
                            # print(dic_i)
                            Item_spec = Item_Spec.objects.filter(**dic_i)
                            # print(Item_Des)
                            if Item_spec:
                                ...
                                updatedic = {'Customer': Customer, 'Project': Project, 'Unit': Unit, 'Phase': Phase,
                                        "Start_time": StartTime, "End_time": EndTime, "Result_time": ResultTime, "Result":'Pass',
                                             'Testitem': Testitem, 'Item_Des': Item_spec.first().Item_Description, 'Tester': request.session.get('user_name'),
                                             'Comments': Comments}

                                # print(update,'1')
                                checktesttime = {'Customer': Customer, 'Project': Project, 'Unit': Unit, 'Phase': Phase, }
                                canupdate = 1
                                if TestLog.objects.filter(**checktesttime):

                                    for i in TestLog.objects.filter(**checktesttime):
                                        if (StartTime < i.Result_time and StartTime > i.Start_time) or (ResultTime < i.Result_time and ResultTime > i.Start_time) or (ResultTime > i.Result_time and StartTime < i.Start_time):
                                            message_se = r"在%s到%s这段时间内，机台:%s %s %s %s 正在做%s 测试" % (
                                            i.Start_time, i.Result_time, Customer, Project, Phase, Unit, Testitem)
                                            actstatus = ""
                                            canupdate = 0
                                            break
                                if canupdate:
                                    print(updatedic)
                                    TestLog.objects.create(**updatedic)
                                return render(request, 'index.html', locals())
                            else:
                                messagei = "Don't have this TestItem, Check again"
                                actstatus = ""
                        ...
                        message_se = r"不符合：StartTime < EndTime < ResultTime"
                    ...
                    if StartTime < EndTime and EndTime < ResultTime:
                        ...
                        if len(Phase) <= 10:
                            ...
                            if Item_spec:
                                ...
                                updatedic = {'Customer': Customer, 'Project': Project, 'Unit': Unit, 'Phase': Phase,
                                             "Start_time": StartTime, "End_time": EndTime, "Result_time": ResultTime,
                                             "Result": 'Fail',
                                             'Testitem': Testitem, 'Item_Des': Item_spec.first().Item_Description,
                                             'Tester': request.session.get('user_name'),
                                             'Comments': Comments}

                                # print(update,'1')
                                checktesttime = {'Customer': Customer, 'Project': Project, 'Unit': Unit,
                                                 'Phase': Phase, }
                                canupdate = 1
                                if TestLog.objects.filter(**checktesttime):
                                    # print(TestLog.objects.filter(**checktesttime))
                                    for i in TestLog.objects.filter(**checktesttime):
                                        if (StartTime < i.Result_time and StartTime > i.Start_time) or (ResultTime < i.Result_time and ResultTime > i.Start_time) or (ResultTime > i.Result_time and StartTime < i.Start_time):
                                            # print('4')
                                            message_se = r"在%s到%s这段时间内，机台:%s %s %s %s 正在做%s 测试" % (
